...
Could software detect the difference through the ISA — does any instruction's
defined behaviour, or any software-readable state, depend on this value?

If two implementations could differ in this value and **no software could ever
observe the difference through the ISA**, it is not an architectural parameter.
It is a microarchitectural implementation detail. Pipeline depth and branch
predictor size are of this kind: real implementation choices, but invisible to
the ISA, and therefore not parameters.
